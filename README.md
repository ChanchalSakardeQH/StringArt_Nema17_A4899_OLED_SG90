...
Every pin this firmware uses is brought out on both the 38-pin and the 30-pin
boards, so the wiring is identical either way — pick whichever matches the
board in front of you.

**38-pin ESP32 WROOM-32** (OLED optional):

![Wiring diagram, 38-pin board](CircuitDiagram/wiring_diagram.png)

**30-pin ESP32 DevKit V1** with the 0.96" OLED dashboard fitted:

![Wiring diagram, 30-pin board](CircuitDiagram/wiring_diagram_30pin.png)

Full size as vector: [`wiring_diagram.svg`](CircuitDiagram/wiring_diagram.svg) and
[`wiring_diagram_30pin.svg`](CircuitDiagram/wiring_diagram_30pin.svg). Both are generated by
`tools/make_diagram.py` from one pin table, so changing a GPIO in the sketch
means editing one line and re-running rather than redrawing.
...
Open **`StringArt_Nema17_A4899_OLED_SG90/StringArt_Nema17_A4899_OLED_SG90/StringArt_Nema17_A4899_OLED_SG90.ino`**
...
```text
StringArt_Nema17_A4899_OLED_SG90/                 repository root
├── StringArt_Nema17_A4899_OLED_SG90/             the Arduino sketch folder — open this one
│   ├── StringArt_Nema17_A4899_OLED_SG90.ino      firmware
...
├── CircuitDiagram/             wiring diagrams, perfboard layout, logo
├── PERFBOARD_BUILD.md          hole-by-hole wire list
├── tools/make_diagram.py       regenerates the schematics from the pin table
├── tools/make_perfboard.py     checks and regenerates the perfboard layout
